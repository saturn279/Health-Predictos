...
#     image=Image.fromarray(image,'RGB')
    ...
    def data_prep(self):
        ...
        negative_dataset_list = os.listdir("Dataset/no/")
        negative_dataset_path= "Dataset/no/"
        positive_dataset_list = os.listdir("Dataset/yes/")
        positive_dataset_path = "Dataset/yes/"
        negative_dataset_array=[]
        positive_dataset_array=[]
        for image_name in negative_dataset_list:
            try:
                print(negative_dataset_path+ image_name)
                image=cv2.imread(negative_dataset_path+ image_name)
                image=Image.fromarray(image,'RGB')
                image=image.resize((240,240))
                negative_dataset_array.append(np.array(image))
                positive_dataset_array.append(0)
            except AttributeError:
                print(negative_dataset_path+ image_name)

        for image_name in positive_dataset_list:
            try:
                image=cv2.imread(positive_dataset_path + image_name)
                image=Image.fromarray(image,'RGB')
                image=image.resize((240,240))
                negative_dataset_array.append(np.array(image))
                positive_dataset_array.append(1)
            except AttributeError:
                print(negative_dataset_path+ image_name)
        negative_dataset_array = np.array(negative_dataset_array)
        positive_dataset_array = np.array(positive_dataset_array)
        self.data = train_test_split(negative_dataset_array, positive_dataset_array, test_size=0.2, shuffle=True, random_state=42)